...
INJECTION_PATTERNS = [
    ...
    r"\b(?:disregard|forget|override|bypass)\s+(?:all\s+)?(?:previous|prior|system|safety|instructions?)",
    ...
    r"\b(?:translate|convert|output|encode).{0,40}(?:prompt|instructions?|config).{0,30}(?:json|xml|yaml|base64|rot13)?",
    ...
    r"\b(?:tiết\s+lộ|tiet\s+lo|cho\s+tôi\s+xem|cho\s+toi\s+xem).{0,30}(?:mật\s+khẩu|mat\s+khau|system\s+prompt|api\s+key)",
]
        ...
        *,
        ...
        if detect_injection(text):
            ...
            return self._block_response(
                "Request blocked: a prompt-injection or secret-extraction pattern was detected."
            )
        if topic_filter(text):
            self.blocked_count += 1
            return self._block_response(
                "Request blocked: VinBank assistant only supports safe banking topics."
            )
        return None
